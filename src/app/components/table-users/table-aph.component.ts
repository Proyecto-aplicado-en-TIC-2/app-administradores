...
export class TableAphComponent implements OnInit {
  ...
  paginated: AphModel[] = []; // Reportes de la página actual
  ...
  itemDeleteTemp = new AphModel();

  // Eliminar un usuario
  openModalDeleteCommunity(item: AphModel) {
  ...
  itemCommunityUpdate = new AphModel();
  ...
  // Eliminar un usuario
  openModalUpdateCommunity(item: AphModel) {
  ...
  updateCommunity() {
    // item actualizado
    let item: AphModel = new AphModel();
    ...
    this.aphService.putById(item).subscribe((value) => {
      // cerrar el modal
  ...
  itemCommunityView = new AphModel();

  openModalViewCommunity(item: AphModel) {
    // Modal para actualizar uno de la comunidad
